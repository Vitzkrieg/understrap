...
(function (factory) {
  ...
  (function ($) {
    ...
    var getMonthHtml = function (month, events) {
      ...
      var prevBtn = $('<button type="button" class="prev" title="Previous"><</button>');
      $(".calendar-actions", header).append('<button type="button" class="today">Today</button>').append(prevBtn).append('<button type="button" class="next" title="Next">></button>');
      header.append('<div class="month-label w-50">' + month.format("MMMM YYYY") + "</div>");
